...
**Azure Storage Practical Guide** — a documentation hub for designing, operating, and troubleshooting Azure Storage — storage accounts, Blob, Files, Queue, Table, redundancy, security, and lifecycle management.

- **Live site**: <https://yeongseon.github.io/azure-storage-practical-guide/>
- **Repository**: <https://github.com/yeongseon/azure-storage-practical-guide>
...
```markdown
![Storage account overview showing a healthy account](../assets/example.png)
...
content_validation:
  ...
  core_claims:
    - claim: "Azure Storage supports LRS, ZRS, GRS, and GZRS redundancy"
...
## Documentation Quality Gate (Quality Gate)

Run `python scripts/validate_doc_quality.py --all` to scan `docs/` for structural and safety defects that the other gates do not cover: missing or mis-ordered tail sections (`## See Also` before `## Sources`), missing required section headings for Best Practices / Operations / Troubleshooting templates, missing `content_validation` metadata on `platform/`, `best-practices/`, and `operations/` pages, Azure CLI code blocks lacking a nearby explanation table or using short flags (`-g`, `-n`), Mermaid blocks missing `diagram-id` metadata, generated placeholder phrases, thin lab sections, broken repository-tracked file references, and real-looking sensitive values (subscription IDs, Application Insights keys, secrets, unmasked GUIDs). In CI the `Validate Documentation Quality` job blocks on **changed Markdown files only** (`--changed-only`, so historical debt never becomes a permanent failure) and runs a full-repo advisory `--all` scan for visibility.

This validator is a **shared core** rule set vendored across the Azure Practical Guide series, not a blind verbatim copy: the core validation logic is identical in every repository, and each repo adjusts only the `Per-repository profile` block at the top of the script (`REQUIRE_CONTENT_VALIDATION`, `SERVICE_LIVE_HOST_RE`, `REPO_DIRS`). For this repository the profile keeps per-file `content_validation` enabled, defines no service-specific live-host rule, and treats local file references outside `REPO_DIRS` (for example a `policy.json` a tutorial tells the reader to author inline, or `./lab-data/...` runtime inputs) as reader-created so they are never false-blocked. This gate is a Prevention item of the cross-repo quality-gate audit ([azure-container-apps-practical-guide#385](https://github.com/yeongseon/azure-container-apps-practical-guide/issues/385)); it is vendored per repository so each guide carries its own script and workflow.
