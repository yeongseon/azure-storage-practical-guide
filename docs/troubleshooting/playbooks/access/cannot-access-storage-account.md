...
content_validation:
  ...
  reviewer: agent
  core_claims:
    - claim: Storage firewalls and virtual network rules can deny requests from disallowed networks.
      source: https://learn.microsoft.com/en-us/azure/storage/common/storage-network-security
      verified: false
    - claim: Private endpoint access requires DNS to resolve the storage endpoint to the private IP path.
